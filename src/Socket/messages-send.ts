...
export const makeMessagesSocket = (config: SocketConfig) => {
	...
	const assertSessions = async (jids: string[], force?: boolean) => {
		...
		if (jidsRequiringFetch.length) {
			...
			const lidResults = pnJids.length ? (await signalRepository.lidMapping.getLIDsForPNs(pnJids)) || [] : []
			const resolvedPnSet = new Set(lidResults.map(a => a.pn))
			const unresolvedPnJids = pnJids.filter(jid => !resolvedPnSet.has(jid))

			const wireJids = [...lidJids, ...lidResults.map(a => a.lid), ...unresolvedPnJids]
