...
export const makeMessagesRecvSocket = (config: SocketConfig) => {
	...
	const sendRetryRequest = async (node: BinaryNode, forceIncludeKeys = false) => {
		...
		const retryParticipant = msgKey.participant || msgKey.remoteJid || undefined

		if (messageRetryManager) {
			if (messageRetryManager.hasExceededMaxRetries(msgId, retryParticipant)) {
				logger.debug({ msgId }, 'reached retry limit with new retry manager, clearing')
				messageRetryManager.markRetryFailed(msgId, retryParticipant)
				return
			}

			const retryCount = messageRetryManager.incrementRetryCount(msgId, retryParticipant)
	...
	const sendMessagesAgain = async (
		...
		for (const id of ids) {
			...
			if (messageRetryManager) {
				...
				if (cachedMsg) {
					...
					messageRetryManager.markRetrySuccess(id, participant)
				}
			}
		...
		if (isJidGroup(remoteJid)) {
			const { [remoteJid]: existingSenderKeyMap } = await authState.keys.get('sender-key-memory', [remoteJid])
			if (existingSenderKeyMap && existingSenderKeyMap[participant]) {
				const updatedSenderKeyMap = { ...existingSenderKeyMap }
				delete updatedSenderKeyMap[participant]
				await authState.keys.set({ 'sender-key-memory': { [remoteJid]: updatedSenderKeyMap } })
			}
		}
